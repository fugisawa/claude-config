...
- **Documentos → PDF** (duas trilhas + uma a cunhar; decide a **densidade de design**, não o tamanho): (1) texto corrido com citações — acadêmico ABNT/APA, relatório sóbrio, apostila — → perfis pandoc `pandoc doc.md -d abnt|apa|eisvogel|docx|html` (kit: `~/.local/share/pandoc` + `~/Documents/md-export-kit/GUIA.md`; gotchas: `learned/pandoc-pdf-pipeline-gotchas`; YAML do doc vence o perfil). (2) Argumento analítico com design editorial (capa, sidebars, dataviz) p/ gestores/circulação → `briefing-designer` (personas) — NUNCA p/ cheatsheet/ficha, o workflow é pesado demais. (3) Artefatos de ESTUDO memoráveis (cheatsheet, guia rápido, ficha de revisão, acessório de meta-aprendizagem — bonitos, nada de "Word de cursinho") → HTML/CSS leve→WeasyPrint, base no CSS do `igepp-aula-reformat`; trilha dedicada ainda a cunhar no manual_estudo. `docx` p/ Word puro; `pdf-processing-pro` genérico; aula IGEPP → `igepp-aula-reformat`.
