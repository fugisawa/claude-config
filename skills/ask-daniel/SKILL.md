...
- **Artigo científico para ACHAR, ABRIR, REGISTRAR, CONFERIR ou PEDIR** (DOI ou URL de editora colados, "me consegue esse paper", "acha o PDF de", "não consigo abrir esse artigo", "texto integral", barreira de pagamento, pedido de Sci-Hub, "escreve pro autor", "os números batem com o artigo?"): `artigos-cientificos` (16/09/2026) — seis comandos (resolver, buscar, abrir, registrar, pedido, conferir), a escada de acesso legítimo em oito degraus (APIs abertas, cópia do autor, pré-publicação, conectores scite/PubMed, navegador da app, dados no OSF, o acesso do perfil e o pedido ao autor), as etiquetas A–D no recibo, o bloco de perfil (CAFe, biblioteca, COMUT, orçamento: vazio, pula) e o parágrafo "Fonte:" que entra no material. O pedido ao autor vira rascunho no conector do Gmail, e nunca envio. Sci-Hub é a rota D, excluída, e a skill diz por quê, com a base legal conferida. Revisão de literatura NÃO é lá: `deep-research`, ou o agente `academic-researcher`, que acha e sintetiza sem Bash; abrir e conferir é nela. Nasceu da conferência de Tharumalingam e col. (2025).
...
- **Concursos**: planejamento/cronograma/check-in/métricas/replanejamento pós-edital: `estrategista-concurso` (o quê/quando/quanto — nunca conteúdo); método/conteúdo (bancas FGV×CEBRASPE, questões, Anki, discursivas, simulados): `concurso-prep`. Aulas em PDF do IGEPP → MD limpo (Obsidian) + PDF imprimível: `igepp-aula-reformat` (instância aterrissada do método genérico). Material de OUTRA origem (apostila, capítulo, norma, paper) → `pdf-to-markdown`. Montar cadernos/simulados no QConcursos (UI Elite ou automação Chrome, filtros e qualidade do lote), extrair/ler resultado de simulado feito (screenshots de gabarito + caderno manuscrito → placar por banca, erros×confiança, divergências) e **auditar uma questão** (gabarito oficial, questão anulada ou desatualizada por mudança de norma/jurisprudência, banca contra a doutrina, comentário de aula que dá a razão errada do gabarito): `qconcursos-simulados`. Fila de estudo por disciplina: **não há skill** — a fila é o artefato `~/manual_estudo/disciplinas/<matéria>/trilha.md`, cuja convenção (formato v2: cada passo = aula IGEPP + par de links QC FGV·Cebraspe; a trilha **materializa**, não re-deriva) vive em `disciplinas/README.md`. Quem manda consumi-la é `estrategista-concurso` (regra-mãe 8: o tópico de todo slot é o primeiro passo não-marcado; nunca inventar sequência). Sinais psicológicos: só nomear e proteger o dia — condução é do acervo web (kit-sobrevivencia-atipica). Manual: `~/.claude/docs/manual-concurso.md`. Aprendizado geral stateful (workspace com learning-records): `/teach`.
...
- **README de repositório** (escrever, refatorar, avaliar): `readme-de-repositorio`. Traz a **decisão de fronteira** — o que fica no README, o que vai para o guia técnico do assistente, o que vai para o mapa de arquitetura, e o que **não vai a documento nenhum porque um comando responde** —, a estrutura recomendada com o estado de cada seção (obrigatória × opcional) e a razão da ordem, seis réguas de navegabilidade **conferíveis** (sumário ancorado acima de 100 linhas pela *Standard Readme*; primeira tela com o primeiro comando, porque 57% do tempo de leitura fica acima da dobra na medição da NN/g; vão máximo sem cabeçalho; profundidade até `###`; cabeçalho com cheiro de informação; tabela de donos completa) mais o comando que as mede, o teste de leitura por contagem de saltos, onze anti-padrões nomeados e o roteiro de refatoração em oito passos. Base empírica: Prana e col. (EMSE 2019, 4.226 seções em 393 repositórios) — o "por quê" existe em só 25,7% dos READMEs e o estado em 21,4%, que é por isso que as duas seções são obrigatórias aqui. Discriminador: **estrutura e fronteira → `readme-de-repositorio`; legibilidade da prosa → agente `revisor-prosa-ptbr`; documentação de API, tutorial ou site de docs → agente `documentation-expert`** (pelo modelo Diátaxis, nada disso é README). Para SKILL.md, e não README, é `writing-great-skills`.
...
- Pesquisa → `research-orchestrator` quando a pergunta é ampla ou vaga (ele clarifica, aloca e sintetiza); os especialistas (`academic-researcher`, `technical-researcher`, `search-specialist`) direto quando o recorte já está claro ou para tarefas delegadas em paralelo.
