"""Extração de texto de PDF de terceiro, imune a marca d'água e mobília repetida.
...
Estratégia: extração por spans do PyMuPDF, filtrando
  ...
  2. mobília repetida (mesmo texto na mesma altura em >=40% das páginas)
     -> rodapés/cabeçalhos de texto (nome da instituição, site, aviso de edição);
...
Uso:
    uv run --with pymupdf python extract_spans.py fonte.pdf saida.txt
    # cortes manuais extras (pontos, opcional):
    uv run --with pymupdf python extract_spans.py fonte.pdf saida.txt --header-cut 80 --footer-cut 90
    # descartar linhas por padrão (repetível), além da mobília detectada:
    ... --drop "^Confidencial" --drop "^Powered by"
"""
import argparse
import re
...
# Assinaturas de gerador que nenhum leitor quer: aparecem uma vez por página e não são
# conteúdo. Ampliável por --drop; deixadas aqui porque são as que mais aparecem na prática.
DROP_DEFAULT = (r"^Powered by TCPDF", r"^Generated by ", r"^Created with ")
...
def furniture_keys(rows, n_pages: int, fraction: float = FURNITURE_MIN_FRACTION) -> set:
    """Chaves (y arredondado, texto) que se repetem em muitas páginas."""
    freq = Counter()
    for pno, y0, _x0, _st, plain in rows:
        freq[(round(y0), plain)] += 1
    threshold = max(3, int(n_pages * fraction))
...
def main() -> None:
    ...
    ap.add_argument("--drop", action="append", default=[], metavar="REGEX",
                    help="descartar linha que casa com o padrão (repetível)")
    ap.add_argument("--furniture-fraction", type=float, default=FURNITURE_MIN_FRACTION,
                    help=f"fração de páginas p/ considerar mobília (default {FURNITURE_MIN_FRACTION})")
    ...
    drop_res = [re.compile(rx) for rx in (*DROP_DEFAULT, *args.drop)]
    rows = collect_lines(doc, args.header_cut, args.footer_cut)
    # mobília calculada no doc INTEIRO, mesmo quando se extrai só um intervalo: a repetição
    # só é visível na amostra completa
    furniture = furniture_keys(rows, len(doc), args.furniture_fraction)
    ...
    for pno in range(p_ini, p_fim + 1):
        ...
        for _pno, y0, _x0, styled, plain in page_rows:
            ...
            if plain in (str(pno), f"{pno}/{len(doc)}", f"- {pno} -",
                ...
                dropped += 1
                continue
            if any(rx.search(plain) for rx in drop_res):
                dropped += 1
                continue
