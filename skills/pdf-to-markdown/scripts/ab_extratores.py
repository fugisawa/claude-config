...
Existe porque a escolha de extrator não é óbvia e muda com o PDF. Numa medição real
(28/07/2026, contra um PDF de cursinho com marca d'água rotacionada), o docling extraiu
~23% mais texto e zero artefato de marca d'água, mas perdeu TODA a ênfase inline — e em
texto legal o negrito carrega sentido. Empate técnico: nenhum domina, e o vencedor depende
do que o SEU material não pode perder. O terceiro braço, pdfplumber, testa a regra "não
confie no dump para tabelas".
...
# Em máquina cujo kernel CUDA seja incompatível com o runtime que o docling carrega, ele
# aborta na importação. Neutralizar a GPU resolve e não muda o resultado da extração.
# Precisa vir ANTES do import.
...
def imprime_placar(resultados: list[Resultado]) -> None:
    ...
    print("  · tabelas detectadas (pdfplumber) testa a rota 1 do passo 4 da skill:")
